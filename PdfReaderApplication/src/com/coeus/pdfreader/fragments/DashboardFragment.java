...
@SuppressLint("NewApi")
public class DashboardFragment extends AuthenTicateParentFragment implements
		OnClickListener {
	View rootView;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(R.layout.fragment_dashboard, container,
				false);
		this.fancyCoverFlow = (FancyCoverFlow) rootView
	...
	private void pdfBooksDataApiCall() {
		...
		try {
			...
			if (AppConstants.isConnectingToInternet(getActivity())) {
				...
				String jasonResponse = sharedPreferences.getString(
						"json_response", "");
				updateData(jasonResponse);
			}
		} catch (Exception e) {
	...
	private void setCoverFlow() {
		this.fancyCoverFlow.setAdapter(new CoverFlowAdapter(
				arrayListPdfFileDetail, getActivity()));
		...
		fancyCoverFlow.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				txtBookTitle.setText(arrayListPdfFileDetail.get(position)
						.getBookTitle());
	...
	private void loadUIComponents() {
		...
		btnDashboardOpenBookmarkList = (Button) rootView
	...
	public void onClick(View v) {
		...
		case R.id.btnOpenFile:
			try {
				...
				DownloadZipPdfFileUtil.createDir(Environment
						.getExternalStorageDirectory().toString(),
						AppConstants.folderName);
				DownloadZipPdfFileUtil.createDir(AppConstants.filePath,
						AppConstants.subFolderName);
				String unzipFileLocation = AppConstants.filePath + "/"
						+ AppConstants.subFolderName + "/";
				String zipFileLocation = AppConstants.filePath
						+ "/"
						+ AppConstants.subFolderName
						+ "/"
						+ arrayListPdfFileDetail.get(coverNumber)
								.getPdfFileName();

				try {
					File file = new File(unzipFileLocation,
							arrayListPdfFileDetail.get(coverNumber)
									.getPdfFileName() + ".pdf");
					if (file.exists()) {
						...
						try {
							...
							Log.e("Open Pdf File Exception",
									"" + e.getStackTrace());
						}

					} else {
						if (AppConstants.isConnectingToInternet(getActivity())) {
							new DownloadZipPdfFileUtil().downloadEventData(
									getActivity(), zipFileLocation,
									unzipFileLocation, arrayListPdfFileDetail
											.get(coverNumber).getBookUrl());
						} else {
							Toast.makeText(getActivity(), noInternetMsg,
	...
	private void showBookMarksListFromDb() {
		...
		try {
			...
			for (int i = 0; i < bookmarksListFromDb.size(); i++) {
				String[] splitedNamePageNum = bookmarksListFromDb.get(i)
						.getBookmarkPageNum().split("_");
				String bookName = splitedNamePageNum[0] + "_"
						+ splitedNamePageNum[2];

				bookName = bookName.substring(0, 1).toUpperCase()
						+ bookName.substring(1).toLowerCase();
				bookmarksNameList.add(bookName);
				Log.e("", "Pages: "
						+ bookmarksListFromDb.get(i).getBookmarkPageNum());
			}
			if (bookmarksNameList.size() > 0) {
				showBookMarksList(bookmarksNameList);
			} else {
				Toast.makeText(getActivity(), noBookmarkFoundMsg,
	...
	private List<BookmarksORM> getBookmarkListFromDataBase() {
		...
		try {
			...
			Log.e("getBookmarkListFromDataBase List Exception",
					"" + e.getStackTrace());
		}
	...
	public void showBookMarksList(final List<String> list_FileName) {
		...
		viewList = getActivity().getLayoutInflater().inflate(
				R.layout.listview_bookmarks, null);
		...
		listViewBookmarks = (ListView) viewList
				.findViewById(R.id.listviewBookmarks);
		listViewBookmarks.setBackgroundResource(R.color.black);

		ArrayAdapter<String> adapterBookmarkList = (new ArrayAdapter<String>(
		...
		listViewBookmarks.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				String[] splitedNamePageNum = list_FileName.get(position)
						.split("_");
				String bookName = splitedNamePageNum[0].toLowerCase()
						+ "_book.pdf";

				String pagenum = splitedNamePageNum[1];

				String filePath = "file://" + AppConstants.filePath + "/"
						+ AppConstants.subFolderName + "/" + bookName;
	...
	public void getAPIResult(String result) {
		...
		if (!result.equals("")) {
			storeSharedPrefData("json_response", jasonResponse);
		}
		updateData(jasonResponse);
	}

	private void updateData(String jsonResponse) {
		try {
			JSONArray jasonPdfArray = new JSONArray(jsonResponse);
			...
			setCoverFlow();
			Log.e("Api Response: ", "" + jsonResponse);
		} catch (Exception e) {
		...
		public void onReceive(Context context, Intent intent) {
			try {
				...
				if (internetStatus.equals("0")) {
					...
					noInternetDialog(getActivity(),
							AppConstants.internetMessage);
					
					
					// Toast.makeText(context, AppConstants.internetMessage,
					// Toast.LENGTH_LONG).show();
					...
					Toast.makeText(context,
							AppConstants.internetConnectedMessage,
							Toast.LENGTH_LONG).show();
